...
def get_bank_account_service(db: AsyncSession = Depends(get_db),
    ...
    return BankAccountService(
        bank_repo=bank_repo,
        detailitem_service=detailitem_service,
        db=db
    )

def get_bank_transaction_service(repository: IBankTransactionRepository = Depends(get_bank_transaction_repository),
    db: AsyncSession = Depends(get_db)) -> IBankTransactionService:
    return BankTransactionService(repository, db)

def get_account_report_service(repository: IAccountReportRepository = Depends(get_account_report_repository))->IAccountReportService:
    return AccountReportService(repository)

def get_db_period_service(db: AsyncSession = Depends(get_async_db),
):
...
def get_trial_balance_service(repository: ITrialBalanceRepository = Depends(get_trial_balance_repository))->ITrialBalanceService:
    return TrialBalanceService(repository)

def get_bank_withdraw_service(repository: IBankWithdrawRepository = Depends(get_bank_withdraw_repository))->IBankWithdrawService:
...
def get_assign_permission_service(repository: IAssignPermissionRepository = Depends(get_assign_permission_repository))->IAssignPermissionService:
    return AssignPermissionService(repository)

def get_login_service(repository: ILoginRepository = Depends(get_login_repository))->ILoginService:
    return LoginService(repository)

def get_role_service(repository: IRoleRepository = Depends(get_role_repository))->IRoleService:
    return RoleService(repository)

def get_permission_service(repository: IPermissionRepository = Depends(get_permission_repository))->IPermissionService:
    return PermissionService(repository)

def get_role_permission_service(repository: IRolePermissionRepository = Depends(get_role_permission_repository))->IRolePermissionService:
    return RolePermissionService(repository)

def get_user_service(repository: IUserRepository = Depends(get_user_repository))->IUserService:
    return UserService(repository)

def get_employee_service(repository: IEmployeeRepository = Depends(get_employee_repository))->IEmployeeService:
    return EmployeeService(repository)

def get_controlitem_service(repository: IControlItemRepository = Depends(get_controlitem_repository))->IControlItemService:
    return ControlItemService(repository)

def get_reportingitem_service(repository: IReportingItemRepository = Depends(get_reportingitem_repository))->IReportingItemService: 
    return ReportingItemService(repository)

def get_detailitem_service(repository: IDetailItemRepository = Depends(get_detailitem_repository))->IDetailItemService: 
    return DetailItemService(repository)

def get_payscalemapping_service(repository: IPayScaleMappingRepository = Depends(get_payscalemapping_repository))->IPayScaleMappingService: 
    return PayScaleMappingService(repository)

def get_generatesalary_service(repository: IGenerateSalaryRepository = Depends(get_generatesalary_repository))->IGenerateSalaryService: 
