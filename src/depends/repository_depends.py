...
def get_customer_receipt_repository(
    db: AsyncSession = Depends(get_db),
) -> ICustomerReceiptRepository:
...
def get_balance_sheet_repository(
    db: AsyncSession = Depends(get_db),
) -> IBalanceSheetRepository:
    return BalanceSheetRepository(db)


def get_trial_balance_repository(
    db: AsyncSession = Depends(get_db),
) -> ITrialBalanceRepository:
    return TrialBalanceRepository(db)


def get_bank_withdraw_repository(
    db: AsyncSession = Depends(get_db),
) -> IBankWithdrawRepository:
    return BankWithdrawRepository(db)


def get_bank_deposit_repository(
    db: AsyncSession = Depends(get_db),
) -> IBankDepositRepository:
...
def get_assign_permission_repository(
    db: AsyncSession = Depends(get_db),
) -> IAssignPermissionRepository:
...
def get_controlitem_repository(
    db: AsyncSession = Depends(get_db),
) -> IControlItemRepository:
    return ControlItemRepository(db)


def get_reportingitem_repository(
    db: AsyncSession = Depends(get_db),
) -> IReportingItemRepository:
