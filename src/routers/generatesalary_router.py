...
from src.dto.generatesalaryresponsedto import (
    GenerateSalaryResponseDTO,
    GenerateSalarySaveResponseDTO,
)
...
@router.post("/insertSalaryInformation", response_model=GenerateSalarySaveResponseDTO)
...
async def get_next_salary_number(
    service: IGenerateSalaryService = Depends(get_generatesalary_service),
):
...
def get_all_monthnames():
    return [
        {"id": item.value, "name": item.name.replace("_", " ").title()}
        for item in MonthName
    ]
